<script lang="ts">
  ...
  import { model, session, mode, role, controller } from '$lib/stores/demo-state';

  let { data }: { data: PageData } = $props();

  // --- Learnosity readiness ---
  // Sentinel: lrn-assess is the root custom element the Items API always renders.
  const LEARNOSITY_SENTINEL = 'lrn-assess';
  ...
  $effect(() => {
    ...
    (async () => {
      try {
        elementModel = await currentController.model(
          currentModel,
          get(session) || {},
          { mode: currentMode, role: currentRole }
        );
      } catch (e) {
      ...
      <div id="pie-container">
        {#if elementModel}
          <pie-element-player
            ...
            model={JSON.stringify(elementModel)}
