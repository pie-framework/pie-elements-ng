 ...
 *
 * Provides the common layout structure with control panels for all player views.
...
$effect(() => {
  if (mode === 'evaluate' && controller && model && session) {
    if (debug) console.log('[player-layout] Calling controller.score()');
    ...
    if (scoreMethod) {
      scoreMethod(model, session, { mode, role: playerRole, partialScoring })
        .then((result: any) => {
          score = result;
          if (debug) console.log('[player-layout] Score result:', result);
        })
        .catch((err: any) => {
          console.error('[player-layout] Scoring error:', err);
          if (debug) score = { error: err.message };
        });
    } else {
      console.warn('[player-layout] Controller has no score or outcome method');
    }
  } else {
    score = null;
  }
...
onMount(async () => {
  try {
    ...
    if (capabilities) {
      ...
      // Try to load configure
      const configureTag = `${elementName}-configure`;
      try {
        await loadElement(`${packageName}/configure`, configureTag, '', debug, true);
      ...
      try {
        await loadElement(`${packageName}/print`, printTag, '', debug, true);
...
function handleModelApply(nextModel: any) {
  ...
  console.log('[player-layout] Model apply requested:', nextModel);
}
</script>
