...
function generateElementImport(element: ElementMetadata, indent: string = ''): string[] {
  ...
  if (controllerInfo.exists) {
    lines.push(
      `${indent}registerController('${elementName}', () => import('${packageSpecifier(elementName, 'controller')}'));`
    );
  }
...
function generateRuntimeFile(elements: ElementMetadata[]): string {
  ...
  lines.push(' *');
  lines.push(' * This file registers all available PIE elements using package specifiers.');
  lines.push(' * We intentionally avoid absolute filesystem paths to keep local-machine');
  lines.push(' * details out of generated files and build artifacts.');
  lines.push(' *');
  lines.push(' * IMPORTANT: import specifiers are static strings so Vite can analyze');
  lines.push(' * and optimize them without leaking machine-specific absolute paths.');
  lines.push(' *');
