 ...
 */
async function maybeRunBuild(repoRoot: string, buildScope: BuildScope): Promise<void> {
...
export async function createStandaloneServer(
  ...
  const { port = 5179, allowRandomPortFallback = true, selfTest = false, config = {} } = options;
  ...
  if (context.config.preBuild && context.config.buildScope) {
    await maybeRunBuild(context.config.repoRoot, context.config.buildScope);
  }
  ...
  // Log server info
  // eslint-disable-next-line no-console
  console.log(`[local-esm-cdn] Serving PIE modules from: ${context.config.repoRoot}`);
  // eslint-disable-next-line no-console
  console.log(`[local-esm-cdn] Listening on: http://localhost:${server.port}`);
  // eslint-disable-next-line no-console
  console.log(`[local-esm-cdn] Rewriting external imports to: ${context.config.esmShBaseUrl}`);
