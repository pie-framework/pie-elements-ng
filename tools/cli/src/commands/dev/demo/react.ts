...
export default class DevDemoReact extends Command {
  ...
  public async run(): Promise<void> {
    ...
    // Resolve element path in React elements
    const elementPath = path.join(process.cwd(), 'packages', 'elements-react', args.element);

    if (!existsSync(elementPath)) {
      this.error(
        `React element not found: ${args.element}\nLooked in: ${elementPath}\n\nDid you mean to use dev:demo:svelte?`
      );
    }
    ...
    this.log(`Starting React demo for ${args.element}...`);
    this.log('');
