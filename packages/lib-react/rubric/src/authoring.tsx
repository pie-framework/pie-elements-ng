...
import { range, takeRight, times } from '@pie-element/shared-lodash';
...
const Row: any = styled('div')(() => ({ display: 'flex', width: '100%', position: 'relative' }));

const EditorDiv: any = styled('div')(({ theme }) => ({ width: '100%', backgroundColor: `${theme.palette.common.white}` }));

const DragIndicatorStyled: any = styled(DragIndicator)(({ theme }) => ({ paddingTop: theme.spacing(1), color: grey[500] }));

const PointsLabel: any = styled(Typography)(({ theme }) => ({
  color: grey[500],
  paddingBottom: theme.spacing(1),
  textTransform: 'uppercase',
}));

const SampleAnswersEditor: any = styled('div')(({ theme }) => ({ paddingLeft: theme.spacing(3) }));
...
const PointMenuWrapper: any = styled('div')(() => ({ position: 'absolute', right: 0 }));
...
export class RawAuthoring extends React.Component {
  ...
  changeRubriclessInstruction: any = (input) => {
    const { value, onChange } = this.props;
    onChange({ ...value, rubriclessInstruction: input });
  };

  changeMaxPoints: any = (maxPoints) => {
  ...
  changeContent: any = (index, content, type) => {
