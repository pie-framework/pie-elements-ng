...
export const closestDroppableKeyboardCoordinates = (event, { active, context, currentCoordinates }) => {
  ...
  for (const [id, container] of droppableContainers) {
    ...
    // that neighbor as "over" even though the item visually landed on the intended
    // target (confirmed live in the equivalent drag-in-the-blank bug this was ported
    // from a fix for).
    const dropPosition = {
      x: rect.left,
