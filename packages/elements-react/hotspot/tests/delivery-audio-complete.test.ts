...
vi.mock('../src/delivery/hotspot/index.js', () => ({ default: () => null }));
...
const { default: Hotspot } = await import('../src/delivery/index.js');

const TAG = 'pie-hotspot-audio-complete-test';
if (!customElements.get(TAG)) {
  customElements.define(TAG, Hotspot as CustomElementConstructor);
}
...
const answeredModel = {};
const answeredSession = () => ({ answers: [{ id: 'shape1' }] });
...
describe('hotspot audio requirement', () => {
  beforeEach(() => {
    vi.useFakeTimers();
  });
...
describe('hotspot prompt audio tracking', () => {
  beforeEach(() => {
    vi.useFakeTimers();
  });
