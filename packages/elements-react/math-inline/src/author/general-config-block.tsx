...
const MATH_FIELD_REGEX = /\\MathQuillMathField\[r\d*\]\{(.*?)\}/g;
...
function prepareForStatic(expression) {
  if (expression) {
    return expression
      .replace(REGEX, () => '\\%response\\%')
      .replace(NEWLINE_LATEX, '\\newline');
  }
}
...
function prepareForModel(expression) {
  if (expression) {
    return expression
      ...
      .replace(MATH_FIELD_REGEX, () => '{{response}}')
...
class GeneralConfigBlock extends React.Component {
  ...
  componentDidMount() {
    this.syncResponseAreasFromExpression();
  }
  ...
  syncResponseAreasFromExpression: any = () => {
