...
<script lang="ts">
  ...
  const resolvedStrategy = $derived(
    normalizeElementPlayerStrategy(strategy, "esm"),
  );
  const resolvedView = $derived(
    resolveElementPlayerView({ mode, view }, "delivery"),
  );
  ...
  async function ensureLoaded() {
    ...
    try {
      ...
      if (
        loaded.controller &&
        loaded.view === "delivery" &&
        loaded.strategy === "iife"
      ) {
