...
export class MMultiscripts implements ToLaTeXConverter {
  ...
  convert(): string {
    ...
    if (childrenLength < 3)
      throw new InvalidNumberOfChildrenError(name, 3, childrenLength, 'at least');

    const baseContent = mathMLElementToLaTeXConverter(children[0]).convert();

    return (
      this._prescriptLatex() +
      this._wrapInParenthesisIfThereIsSpace(baseContent) +
      this._postscriptLatex()
    );
  }

  private _prescriptLatex(): string {
    const { children } = this._mathmlElement;
    let sub: MathMLElement | undefined;
    let sup: MathMLElement | undefined;
