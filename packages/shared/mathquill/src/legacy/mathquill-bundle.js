...
(() => {
   ...
   *   sendMethod(obj2, 4); // => 6
   */
  var send = variadic((method, args) =>
    variadic((obj, moreArgs) => {
  ...
  function iterator(generator) {
    return variadic(function (fn, args) {
      ...
      var yield_ = (obj) => fn.apply(obj, [obj].concat(args));
  ...
  var P = ((prototype, ownProperty, undefined) => {
  ...
  var Point = P(function (_) {
    ...
    this.copy = (pt) => Point(pt.parent, pt[L], pt[R]);
  });
  ...
  var Node = P(function (_) {
    ...
    _.selectChildren = (leftEnd, rightEnd) => Selection(leftEnd, rightEnd);
...
JS environment could actually contain many instances. */
  ...
  var Cursor = P(Point, (_) => {
    ...
    _.jumpUpDown = function (from, to) {
      ...
      var cached = this.upDownCache[to.id];
      if (cached) {
    ...
    _.offset = function () {
      ...
      var offset = this.jQ.removeClass('mq-cursor').offset();
  ...
  var Selection = P(Fragment, (_, super_) => {
    _.init = function () {
  ...
  function getInterface(v) {
    ...
    function config(currentOptions, newOptions) {
      ...
      for (var name in newOptions)
        if (Object.hasOwn(newOptions, name)) {
    ...
    var AbstractMathQuill = (APIClasses.AbstractMathQuill = P(Progenote, (_) => {
    ...
    APIClasses.EditableField = P(AbstractMathQuill, (_, super_) => {
  ...
  var saneKeyboardEvents = (() => {
    ...
    return function saneKeyboardEvents(el, handlers) {
      ...
      target.bind('keydown keypress input keyup focusout paste', (e) => {
        checkTextarea(e);
      });
  ...
  Controller.open(function (_) {
    ...
    optionProcessors.leftRightIntoCmdGoes = (updown) => {
  ...
  var latexMathParser = (() => {
    ...
    var variable = letter.map((c) => Letter(c));
    var symbol = regex(/^[^${}\\_^]/).map((c) => VanillaSymbol(c));

    // Compound commands e.g. \not\subset
    var compound = regex(/^\\not\\[a-z]+/).then(
      (cmp) =>
        (CompoundCmds[cmp] && CompoundCmds[cmp]().parser()) ||
    ...
    var optMathBlock = string('[')
      .then(
        mathBlock
          .then((block) => (block.join('latex') !== ']' ? succeed(block) : fail()))
          .many()
          .map(joinBlocks)
          .skip(optWhitespace)
      )
  ...
  Controller.open((_, super_) => {
    ...
    _.parseLatex = (str) =>
      latexMathParser.parse(str).postOrder('finalizeTree', Options.p).join('latex').trim();

    optionProcessors.maxDepth = (depth) => (typeof depth === 'number' ? depth : undefined);
  ...
  Controller.open((_) => {
    ...
    _.delegateMouseEvents = function () {
      ...
      this.container.bind('mousedown.mathquill', (e) => {
  ...
  var MathCommand = P(MathElement, (_, super_) => {
    ...
    _.placeCursor = function (cursor) {
      ...
      cursor.insAtRightEnd(
        this.foldChildren(this.ends[L], (leftward, child) =>
          leftward.isEmpty() ? leftward : child
        )
      );
    };
  ...
  */
  ...
  var TextBlock = P(Node, (_, super_) => {
    ...
    _.createLeftOf = function (cursor) {
      ...
      if (this.replacedText)
        for (var i = 0; i < this.replacedText.length; i += 1)
          this.write(cursor, this.replacedText.charAt(i));
    };
    ...
    function fuseChildren(self) {
      ...
      var textPcDom = self.jQ.contents().filter((i, el) => el.nodeType === 3)[0];
  ...
  var RootMathCommand = P(MathCommand, (_, super_) => {
  ...
  var RootTextBlock = P(RootMathBlock, (_, super_) => {
  ...
  var LatexCommandInput = (CharCmds['\\'] = P(MathCommand, (_, super_) => {
    _.ctrlSeq = '\\';
    _.replaces = function (replacedFragment) {
...
*/
  ...
  var Digit = P(VanillaSymbol, (_, super_) => {
    _.createLeftOf = function (cursor) {
      if (
  ...
  var Variable = P(Symbol, (_, super_) => {
    _.init = function (ch, html) {
  ...
  var Letter = P(Variable, (_, super_) => {
    _.init = function (ch) {
    ...
    _.autoUnItalicize = function (opts) {
      ...
      Fragment(l[R] || this.parent.ends[L], r[L] || this.parent.ends[R]).each((el) => {
      ...
      outer: for (
        ...
        for (var len = min(autoOps._maxLength, str.length - i); len > 0; len -= 1) {
          ...
          if (Object.hasOwn(autoOps, word)) {
            for (var j = 0, letter = first; j < len; j += 1, letter = letter[R]) {
              letter.italicize(false);
              var last = letter;
            }

            var isBuiltIn = Object.hasOwn(BuiltInOpNames, word);
            first.ctrlSeq = (isBuiltIn ? '\\' : '\\operatorname{') + first.ctrlSeq;
            last.ctrlSeq += isBuiltIn ? ' ' : '}';
            if (Object.hasOwn(TwoWordOpNames, word)) last[L][L][L].jQ.addClass('mq-last');
  ...
  var OperatorName = P(Symbol, (_, super_) => {
    _.init = function (fn) {
      this.ctrlSeq = fn;
    };
    _.createLeftOf = function (cursor) {
  ...
  for (var fn in AutoOpNames)
    if (Object.hasOwn(AutoOpNames, fn)) {
      LatexCmds[fn] = OperatorName;
    }
  LatexCmds.operatorname = P(MathCommand, (_) => {
    _.createLeftOf = noop;
    _.numBlocks = () => 1;
    _.parser = () => latexMathParser.block.map((b) => b.children());
  });

  LatexCmds.f = P(Letter, (_, super_) => {
    _.init = function () {
  ...
  var NonSymbolaSymbol = P(Symbol, (_, super_) => {
    _.init = function (ch, html) {
  ...
  var LatexFragment = P(MathCommand, (_) => {
    _.init = function (latex) {
      this.latex = latex;
    };
    _.createLeftOf = function (cursor) {
  ...
  var PlusMinus = P(BinaryOperator, (_) => {
  ...
  var Inequality = P(BinaryOperator, (_, super_) => {
  ...
  var Equality = P(BinaryOperator, (_, super_) => {
    _.init = function () {
      super_.init.call(this, '=', '=');
    };
    _.createLeftOf = function (cursor) {
  ...
  LatexCmds.vartriangleright = LatexCmds['⊳'] = bind(
    VanillaSymbol,
    '\\vartriangleright ',
    '&#8883;'
  );
  ...
  LatexCmds.ntrianglerighteq = LatexCmds['⋭'] = bind(
    VanillaSymbol,
    '\\ntrianglerighteq ',
    '&#8941;'
  );
  ...
  LatexCmds.rightthreetimes = LatexCmds['⋌'] = bind(
    BinaryOperator,
    '\\rightthreetimes ',
    '&#8908;'
  );
  ...
  var Style = P(MathCommand, (_, super_) => {
  ...
  var XArrowStyle = P(MathCommand, (_, super_) => {
    _.init = function (ctrlSeq, tagName, attrs) {
      this.args = arguments;
      this.htmlTemplate =
        '<' +
        ...
        '<' +
        tagName +
        ' class="mq-xarrow-over">&0</' +
        tagName +
        '>' +
        '<' +
        tagName +
        ' class="mq-xarrow-under"></' +
        tagName +
        '>' +
        '</' +
        tagName +
    ...
    _.parser = function () {
      return latexMathParser.optBlock
        .then((optBlock) =>
          latexMathParser.block.map((block) => {
            var withSub = XArrowWithSub.apply(null, this.args);
  ...
  var XArrowWithSub = P(XArrowStyle, (_, super_) => {
    _.init = function (ctrlSeq, tagName, attrs) {
      this.htmlTemplate =
        '<' +
        tagName +
        ...
        '<' +
        tagName +
        ' class="mq-xarrow-over">&1</' +
        tagName +
        '>' +
        '<' +
        tagName +
        ' class="mq-xarrow-under">&0</' +
        tagName +
        '>' +
        '</' +
        tagName +
  ...
  var OverLineStyleGenerator = (className) => {
    var arrows =
      '<span class="' +
      className +
      '-inner-right">g</span><span class="' +
      className +
      '-inner-left">h</span>';

    return P(MathCommand, (_, super_) => {
      _.init = function (ctrlSeq, tagName, attrs) {
        super_.init.call(
          this,
          ctrlSeq,
  ...
  var LongDivisionStyle = P(MathCommand, (_, super_) => {
    _.init = function (ctrlSeq, tagName, attrs) {
      super_.init.call(
        this,
        ctrlSeq,
        '<' +
          ...
          ' ' +
          attrs +
          '><span class="mq-longdiv-curve-border">)</span><span class="mq-longdiv-inner"><span class="mq-empty">&0</span></' +
          tagName +
          '></span>'
      );
    };
  ...
  var EditMatrixRowStyle = P(MathCommand, (_, super_) => {
    _.init = function (ctrlSeq, tagName, attrs) {
      super_.init.call(
        this,
        ctrlSeq,
        '<span class="mq-matrix">' +
  ...
  var DotRecurringDecimalStyle = P(MathCommand, (_, super_) => {
  ...
  var EditMatrixColStyle = P(MathCommand, (_, super_) => {
    _.init = function (ctrlSeq, tagName, attrs) {
      super_.init.call(
        this,
        ctrlSeq,
  ...
  var BiggerSymbolStyle = (className, content) =>
    P(Symbol, (_, super_) => {
      _.init = function (ctrlSeq, tagName, attrs) {
        super_.init.call(
          this,
  ...
  LatexCmds.overline = LatexCmds.bar = bind(
    OverLineStyleGenerator('mq-overline'),
    '\\overline',
    'span',
    'class="mq-non-leaf mq-overline"'
  );
  LatexCmds.longdiv = bind(
    LongDivisionStyle,
    '\\longdiv',
    'span',
    'class="mq-non-leaf mq-longdiv"'
  );
  ...
  LatexCmds.xleftarrow = bind(
    XArrowStyle,
    '\\xleftarrow',
    'span',
    'class="mq-non-leaf mq-xarrow mq-arrow-left"'
  );
  LatexCmds.xrightarrow = bind(
    XArrowStyle,
    '\\xrightarrow',
    'span',
    'class="mq-non-leaf mq-xarrow mq-arrow-right"'
  );
  ...
  LatexCmds.dot = P(MathCommand, (_, super_) => {
    _.init = function () {
      super_.init.call(
        this,
        '\\dot',
  ...
  LatexCmds.ngtr = LatexCmds['≯'] = bind(
    BiggerSymbolStyle('mq-ngtr', '&#8815;'),
    '\\ngtr ',
    'span',
    'class="mq-non-leaf mq-ngtr"'
  );
  LatexCmds.nless = LatexCmds['≮'] = bind(
    BiggerSymbolStyle('mq-nless', '&#8814;'),
    '\\nless ',
    'span',
    'class="mq-non-leaf mq-nless"'
  );
  LatexCmds.dot = bind(DotRecurringDecimalStyle, '\\dot');

  LatexCmds.parallelogram = bind(
    BiggerSymbolStyle('mq-parallelogram', '&#9649;'),
    '\\parallelogram ',
    'span',
    'class="mq-non-leaf mq-parallelogram"'
  );
  ...
  var TextColor = (LatexCmds.textcolor = P(MathCommand, (_, super_) => {
  ...
  var Class = (LatexCmds['class'] = P(MathCommand, (_, super_) => {
    _.parser = function () {
      var string = Parser.string,
        regex = Parser.regex;
  ...
  var SupSub = P(MathCommand, (_, super_) => {
    _.ctrlSeq = '_{...}^{...}';
    _.createLeftOf = function (cursor) {
      ...
      for (var dir = L; dir; dir = dir === L ? R : false) {
        if (this[dir] instanceof SupSub) {
          ...
          for (var supsub = 'sub'; supsub; supsub = supsub === 'sub' ? 'sup' : false) {
            ...
            this.placeCursor = ((dest, src) => (cursor) => {
    ...
    _.addBlock = function (block) {
      ...
      for (var i = 0; i < 2; i += 1)
        ((cmd, supsub, oppositeSupsub, updown) => {
  ...
  LatexCmds.subscript = LatexCmds._ = P(SupSub, (_, super_) => {
    _.supsub = 'sub';
    _.htmlTemplate =
  ...
  var SummationLimitNotation = P(MathCommand, (_, super_) => {
    _.parser = function () {
      var string = Parser.string;
      var optWhitespace = Parser.optWhitespace;
      var succeed = Parser.succeed;
      var block = latexMathParser.block;
      var blocks = (this.blocks = [MathBlock(), MathBlock()]);
      for (var i = 0; i < blocks.length; i += 1) {
        blocks[i].adopt(this, this.ends[R], 0);
      }
  ...
  var SummationNotation = P(SummationLimitNotation, (_, super_) => {
    _.init = function (ch, html) {
      var htmlTemplate =
  ...
  LatexCmds.lim = LatexCmds.limit = P(SummationLimitNotation, (_, super_) => {
    _.init = function () {
      var htmlTemplate =
  ...
  LatexCmds.underset = P(MathCommand, (_, super_) => {
    _.ctrlSeq = '\\underset';
    _.htmlTemplate =
  ...
  LatexCmds.overset = P(MathCommand, (_, super_) => {
    _.ctrlSeq = '\\overset';
    _.htmlTemplate =
  ...
  var Hat = (LatexCmds.hat = P(MathCommand, (_, super_) => {
    _.ctrlSeq = '\\hat';
    _.htmlTemplate =
      '<span class="mq-non-leaf">' +
  ...
  var NthRoot = (LatexCmds.nthroot = P(SquareRoot, (_, super_) => {
    _.htmlTemplate =
  ...
  var DiacriticAbove = P(MathCommand, (_, super_) => {
  ...
  var LrnCuberoot = (LatexCmds.lrncuberoot = P(SquareRoot, (_, super_) => {
  ...
  var Abs = (LatexCmds.abs = P(MathCommand, (_) => {
    _.ctrlSeq = '\\abs';
    _.htmlTemplate = '<span class="mq-abs mq-non-leaf"><span class="mq-empty-box">&0</span></span>';
    _.textTemplate = ['|', '|'];
  }));

  var LrnPlaceholder = (LatexCmds.lrnplaceholder = P(MathCommand, (_, super_) => {
  ...
  var LrnExponent = (LatexCmds.lrnexponent = P(MathCommand, (_, super_) => {
  ...
  var LrnSquaredExponent = (LatexCmds.lrnsquaredexponent = P(MathCommand, (_, super_) => {
  ...
  var LrnSubscript = (LatexCmds.lrnsubscript = P(MathCommand, (_, super_) => {
  ...
  var Bracket = P(P(MathCommand, DelimsMixin), (_, super_) => {
  ...
  LatexCmds.left = P(MathCommand, (_) => {
    _.parser = () => {
      var regex = Parser.regex;
      var string = Parser.string;
      var succeed = Parser.succeed;
  ...
  LatexCmds.right = P(MathCommand, (_) => {
    _.parser = () => Parser.fail('unmatched \\right');
  });

  var Binomial =
    (LatexCmds.binom =
    LatexCmds.binomial =
      P(P(MathCommand, DelimsMixin), (_, super_) => {
        _.ctrlSeq = '\\binom';
        _.htmlTemplate =
          '<span class="mq-non-leaf">' +
          '<span class="mq-paren mq-scaled">(</span>' +
  ...
  var Choose = (LatexCmds.choose = P(Binomial, (_) => {
    _.createLeftOf = LiveFraction.prototype.createLeftOf;
  }));

  LatexCmds.editable = // backcompat with before cfd3620 on #233
    LatexCmds.MathQuillMathField = P(MathCommand, (_, super_) => {
  ...
  var Embed = (LatexCmds.embed = P(Symbol, (_, super_) => {
      ...
      return string('{')
        ...
        .then((name) => {
          ...
          return string('[')
            ...
            .skip(string(']'))
            .or(succeed())
            .map((data) => this.setOptions(EMBEDS[name](data)));
        });
    };
  }));
   ...
   * to MathQuill.
   */
  LatexCmds.MathQuillResponseContainer = P(MathCommand, (_, super_) => {
    _.ctrlSeq = '\\MathQuillResponseContainer';
    _.htmlTemplate =
      '<span class="lrn-mq-response-container">' +
      '<marker class="lrn-qe-keyboard-i-response"></marker>' +
      '<content>Response</content>' +
      '</span>';
    _.latex = () => '\\MathQuillResponseContainer';
    _.text = function () {
      return this.ends[L].text();
    };
  });
   ...
   */
  LatexCmds.MathQuillResponseContainerIcon = P(MathCommand, (_, super_) => {
    _.ctrlSeq = '\\MathQuillResponseContainerIcon';
    _.htmlTemplate =
      '<span class="lrn-mq-response-container-icon">' +
      '<span class="lrn-qe-keyboard-i-response"></span>' +
      '</span>';
    _.latex = () => '\\MathQuillResponseContainerIcon';
    _.text = function () {
      return this.ends[L].text();
    };
  });

  var Matrix = (LatexCmds.matrix = P(MathCommand, (_, super_) => {
    var MatrixCell = P(MathBlock, (_, super_) => {
      ...
      _.deleteOutOf = function (dir, cursor) {
        ...
        this.parent.backspace(this, dir, cursor, () => {
          ...
          return super_.deleteOutOf.apply(this, args);
        });
      };
    });
    ...
    _.createBlocks = function () {
      ...
      this.htmlTemplate.replace(/&\d+/g, (match, index) => {
        var row = this.htmlTemplate.substring(0, index).match(/<tr[^>]*>/gi).length - 1;
    ...
    _.parser = function () {
      ...
      return regex(rgxContents).then((a) => {
        ...
        var numRows = Math.min(rows.length, this.maximum.rows);
        var numColumns = 0,
          columns,
          i;
    ...
    _.finalizeTree = function () {
      ...
      if (table.length) {
        ...
        table.removeClass((index, classes) => {
    ...
    _.deleteCell = function (cell) {
      ...
      if (!isLastBlock) {
        ...
        indexInRow = Math.min(
          indexInRow,
          this.jQ.find('tr').eq(indexInColumn).find('td').length - 1
        );
        otherBlock =
          Node.byId[this.jQ.find('tr').eq(indexInColumn).find('td').eq(indexInRow).attr(mqBlockId)];
      }
  ...
  LatexCmds.pmatrix = P(Matrix, (_, super_) => {
    _.ctrlSeq = '\\pmatrix';
  ...
  LatexCmds.bmatrix = P(Matrix, (_, super_) => {
    _.ctrlSeq = '\\bmatrix';
  ...
  LatexCmds.vmatrix = P(Matrix, (_, super_) => {
    _.ctrlSeq = '\\vmatrix';
