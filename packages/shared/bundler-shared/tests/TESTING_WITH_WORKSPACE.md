...
Since the bundler is designed to download packages from NPM, but local workspace packages are easier to validate while upstream NPM dependencies are broken, we need a way to test with local packages.
...
- ✅ Avoid old broken git dependency paths
