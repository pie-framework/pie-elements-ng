...
// This repo publishes under two scopes; both must be writable for a release to complete.
const SCOPES = ['pie-element', 'pie-lib'];
...
const fail = (message, details) => {
  ...
  for (const scope of SCOPES) {
    console.error(
      `- Verify org access with \`npm org ls ${scope} --registry=https://registry.npmjs.org/\``
    );
  }
  process.exit(1);
};

const getErrorDetails = (error) => error?.stderr?.toString?.() || error?.message || String(error);
...
const tryInteractiveLogin = () => {
  ...
  console.log(`[publish-auth] npm auth missing; starting interactive login for ${REGISTRY}`);
    ...
    if (AUTO_LOGIN_ENABLED && isAuthError(details)) {
      ...
      if (loginWorked) {
        try {
          ...
          fail(`${failureMessage} (after interactive login).`, getErrorDetails(retryError));
        }
      }
    }
...
try {
  ...
  console.log(
    `[publish-auth] npm auth OK as "${username}" with registry "${REGISTRY}" and ${SCOPES.map(
      (s) => `@${s}`
    ).join(' + ')} org access`
  );
} finally {
