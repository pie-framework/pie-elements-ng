...
async function login() {
  ...
  try {
    ...
    const authResponse = await fetch(`${VERDACCIO_URL}/-/user/org.couchdb.user:${username}`, {
      method: 'PUT',
      headers: {
...
async function start() {
  ...
  if (await waitForVerdaccio()) {
    ...
    console.log('   Run demos: bun run dev:demo:svelte');
  } else {
...
async function check() {
  ...
  if (!responding) {
    ...
    console.error('📝 Check logs: bun run registry:logs');
    console.error('📝 Restart: bun run registry:stop && bun run registry:start\n');
    process.exit(1);
  }
...
async function status() {
  ...
  if (containerRunning) {
    try {
      const logsResult = await $`docker compose logs --tail=5 verdaccio`.quiet();
      console.log('\nRecent logs:');
      console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
...
switch (command) {
  ...
  case 'publish':
    ...
    break;
  default:
    console.error('Usage: bun scripts/verdaccio.ts <start|stop|check|status|login|publish>');
    process.exit(1);
}
